...
academic, they can sometimes feel like they're tested. So it's always trying to say like, yeah, this is subjective; I'm just interested in your thoughts."}. This can allow for more open answers, as there is less emphasis on stating a 'correct' answer and the respondent may feel more at ease to say whatever comes into their head.
...
In order to put the codes into a standard format, opinionated code formatters were applied to each, with \textit{black} [@black] being applied to the Python code and \textit{styler} [@styler] being to the R. Screenshots showing the implementation of these are seen in appendix [?].

\section{Analysis}
The format of the interviews were recorded zoom meetings, for which consent was given by each respondent prior to starting the recordings. After the interviews were completed and recordings obtained, the online transcription software \textit{Trint} was used to transcribe the interviews for analysis.

These transcriptions can be found in appendix [?]. Also found in this appendix is a summary of responses relating to each question.

The implementation of the survey ended up more towards the structured end of 'semi-structured' due to nerves of the interviewer and too strong a focus on the set questions, and if performed again more focus would be placed on exploration and asking for further expansions from respondents.
...
with Dr. Palmer, Dr. Glynatsi, Dr. Knight and Henry  showing a Python bias, but highlighting some R familiarity; "I am familiar with both languages, but I am very biased towards Python because it's the language I use most.", "I mainly use Python, but very recently I've started using R for a project I'm working on.", "I use Python mostly, but I'm fairly new at R.", "So I'm a lot more familiar with Python than I am with R or a lot more fluent in Python than I am with R".

One respondent showed a particularly strong R bias, stating "I'm an R person. I wrote a book on R, so".  

and went on to mention Hadley Wickham's book on the Tidyverse and graphics, which was then been explored and discussed in this report. When talking about the book, the respondent comments that \textit{"the definite opinion on graphics in particular and how to present them"} is interesting.

It was found that all respondents' reasoning behind the language biases or preferences were based on the language they use the most, and thus which they are more comfortable in, as opposed to any strong opinions against the less favoured language. It should be noted that programmers do tend to hold certain conscious or subconscious loyalties towards the language they favour, which will be considered when looking at opinions regarding each language.


In addition, this respondent went on to discuss how ggplot is \textit{"ggplot has essentially its own programming language."}. However, it is the fault of the interviewer here to not ask for an expansion on this. The respondent also mentions Julia as a programming language, stated as being designed for numerical work and \textit{"much faster than both R and Python, but still not too hard to use."}. In particular, 

In future research, Julia could potentially be discussed in a similar way to R and Python have been in this paper. 

When asked about initial comments on the code, some general points that came up were the fact that the codes are
