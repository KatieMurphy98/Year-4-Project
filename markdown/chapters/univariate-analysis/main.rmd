...
In general, truncating the y-axis had less of an effect than anticipated. In question 1, \textit{"Approximately many times would you say the ‘Salmon Ladder’ was used?"}, the overall mean for the truncated plot responses ($\bar{x} = 41.35$) as compared to the mean of the control plot responses ($\bar{x}=41.21$) shows a small difference, with the average perceived value of the bar being slightly higher for the truncated plot. The median for both of these is 41. Performing a dependent-samples sign test comparing these two sets of responses confirms that there is no significant difference ($p = 0.1877$). However, the one sample sign tests show that there is not sufficient evidence to suggest the control plot responses differ from the true value of 41 ($p = 0.1214$), but there is evidence to accept the hypothesis that the truncated plot responses differ from the true value ($p = 0.0026$). Considering now the R and Python subgroups, comparing the two by eye, there appears to be no significant difference in average values for the truncated plot, with means of $\bar{x}=41.57$ and $\bar{x}=41.14$, respectively. A sign test shows sufficient evidence that the R responses differ from the true value ($p = 0.0004$), whereas there is insufficient evidence when applying a MWW test to the Python responses ($p = 0.718).
...
Finally, comparing the distributions between the three sets of responses, for which we only consider responses for the first shown plots, we see similar distributions to the whole population, showing that the plot order has minimal impact here. 
...
A comparison between languages for each plot type reveals that the language has limited effect on the responses for the control and truncated plots, with MWW showing the difference between the two languages to be insignificant ($p=0.9105$ and $p=0.9105$), in fact the means for the truncated plot are 5.89 and 5.84, respectively for R and Python; very close. There is evidence to show that the difference between the R and Python versions of the logarithmic plot is significant ($p=0.00096$, $\text{mean}_R = 4.263$, $\text{mean}_{Py} = 2.969$). The distributions for the two language subsets are shown below. 
...
ggplot(stats_2_py)+
  ...
  scale_x_discrete(breaks = c("1", "2", "3", "4", "5", "6", "7"), labels = c("1", "2", "3", "4", "5", "6", "7"), limits = c("1", "2", "3", "4", "5", "6", "7"))
```
To conclude, it appears again that truncating the bars doesn't have a particularly significant difference, conversely to the hypothesis that it would cause overestimation of differences. The logarithmic scale, however, does result in respondents perceiving the difference between the two bars as smaller than for the control. 
...
The distribution of the truncated plot appears almost normal, with the control plot skewed slightly more to the left, and then the logarithmic skewed further to the left than this. The means for these three are 3.129, 3.3.77 and 2.229 with medians of 3, 4 and 2, respectively for the control, truncated and logarithmic. The ranges for these are all similar, with the truncated plot responses sitting in $[2, 7]$, the control responses in $[1, 7]$, and the logarithmic in $[1, 6]$, with variances of 1.309, 1.157 and 1.599. All of this points towards the respondents perceiving the differences in the truncated plot as lower than the control plot and the logarithmic plot higher, confirmed by sign tests for the control and truncated comparison ($p=9.248e-06$) and logarithmic and truncated comparison ($1.17e-10$). MMW is used for the control and logarithmic comparison ($p = 2.633e-06$). This is similar to the conclusions from the previous question, but with the difference in responses from the control to the truncated plot being more significant.




Now for the previous two questions, consider the difference in means for 
```{r}
