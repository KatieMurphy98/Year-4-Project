...
See appendix 5 for tables showing summary statistics and p-values.
...
```{r plt1, fig.cap = "Density plot showing distributions of responses regarding the control and truncated plots for the question 1"}
ggplot() +
...
```{r plt2, fig.cap = "Bar plot showing distributions of responses regarding the control and truncated plots for question 2"}
ggplot(stats_2)+
...
Question 3 of part 1, \textit{'Approximately how much more than 'Quintuple Steps' would you say 'Salmon Ladder' was used?'}, asks a similar question to question 2, but asks respondents to judge the difference for bars on opposite ends of the plot as opposed to next to each. Again, the by eye comparison shows not a massive difference between distributions of responses for the control ($\bar{x}=3.12$, $\tilde{x}=3$) and truncated ($\bar{x}=3.12$, $\tilde{x}=3$) plots, although the sign test shows that the there is evidence to suggest that the truncated plot responses are in fact on average greater than for the control plot ($p=4.624e-06$). figure 3.3 shows the distribution of responses.
...
```{r plt3, fig.cap = "Bar plot showing distributions of responses regarding the control and truncated plots for question 3"}
ggplot(stats_3)+
  geom_bar(aes(x=resp, group = type, fill = type), position = 'dodge') +
  ylab("Number of Respondents")+
  xlab("Response")+
...
```{r plt4, fig.cap = "Density plot showing distributions of responses regarding the control and truncated plots for the question 1"}
plot_grid(p, q)
```

For question 2 it is similarly seen that the language used does not have a statistically significant impact on the response for the truncated plot, with means $5.500$ and $5.187$, and medians $6$ and $5$ respectively for R and Python for the control plot, and means $5.98$ and $5.84$ both with median $6$ for the truncated. Comparative testing with MWW gives $p=0.2199$ for the control plot and $0.9105$ for the truncated. Thus, the scale granulation or any other differing aspect of the plots does not seem to have a significant effect. See figure 3.5 for the distributions.
...
```{r plt5, fig.cap = "Bar plot showing distributions of responses regarding the control and truncated plots for question 2, for the R and Python subgroups"}
plot_grid(p, q)
```
...
```{r plt6, fig.cap = "Bar plot showing distributions of responses regarding the control and truncated plots for question 3, for the R and Python subgroups"}
plot_grid(p, q)
```
...
```{r plt7, fig.cap = "Density plot showing distributions of responses regarding the control and logarithmic scaled plot, after removing values of greater or equal to 1000"}
plot_grid(p, q)
```
...
```{r plt8, fig.cap = "Bar plot showing distributions of responses regarding the control and logarithmic plots for question 2"}
ggplot(stats_2)+
  geom_bar(aes(x=resp, group = type, fill = type), position = 'dodge') +
  ylab("Number of Respondents")+
  xlab("Response")+
...
There is evidence to show that the difference between the R and Python versions of the logarithmic plot is significant ($p=0.00096$, $\bar{x}_R = 4.263$, $\bar{x}_{Py} = 2.969$). The distributions for the two language subsets are shown in figure 3.9.
...
```{r plt9, fig.cap = "Bar plots showing distributions of responses regarding the control and logarithmic plots for question 2, separated by language"}
plot_grid(p, q)
```


In regard to question 3, see again figure 3.10 for the plotted distributions. 
...
```{r plt10, fig.cap = "Bar plots showing distributions of responses regarding the control and logarithmic plots for the question 3"}
ggplot(stats_2)+
  geom_bar(aes(x=resp, group = type, fill = type), position = 'dodge') +
  ylab("Number of Respondents")+
  xlab("Response")+
...
figure 3.11 shows the distributions for R and Python subgroups.
...
```{r plt11, fig.cap = "Bar plots showing distributions of responses regarding the control and logarithmic plots for the question 2, separated by language"}
plot_grid(p, q)
```
...
Now take $\bar{x}_{control} - \bar{x}_{truncated}$ and $\bar{x}_{control} - \bar{x}_{logarithmic}$ for each of questions 2 and 3, which is shown in table 3.1. 
```{r}
...
```
This again shows that the responses for the truncated plot were in general rated higher than the control plot responses, and also that the effect was more significant for the bars on opposite ends of the plot as compared to the bars next to each other. The opposite is true for the logarithmic plot responses; on average they were rated lower than the control plot, but this was greatly more significant for the bars next to each other, as opposed to the truncated plot. figure 3.12 shows this visually.
...
```{r plt12, fig.cap= "Bar plot giving a visual representation of the table"}
ggplot()+
...
```
Looking at the means and medians, it doesn't seem like there is that much of a difference in perception of the differences between the three aspect ratios, as displayed in table 3.2.
...
```{r tab2}
kable(tab, digits=3, caption = "Table showing means and medians") %>%
  kable_styling(latex_options = "hold_position")
```
...
Now looking at figure 3.13, showing the three distributions. There isn't an immediately obvious difference in distributions, but on closer inspection it can be seen that the orange "Wide" bars dominate over the three for the range [2, 5], and the purple "Narrow" dominated for the response of 7, following the above analysis of summary statistics. 
...
```{r plt13, fig.cap = "Bar plots showing distributions of responses regarding the three plots"}
ggplot(data = stats)+
  geom_bar(aes(x=resp, group=type, fill=type), position = 'dodge')+
  theme_classic()+
...
There was a fairly strong consensus that in general that a rating of 6 was applicable to all three plots.
...
```{r plt14, fig.cap = "Box plots showing distributions of responses regarding the three plots"}
...
At least $50\%$ of respondents placed the difference in the range $[2, 4]$ for all three plots, showing that they believed the difference was small to moderate, and this didn't change depending on the plot type, and thus for the bars further apart from each other, changing the aspect ratio does not appear to make much of a difference. The overall distributions are shown in the figure 3.15.

```{r plt15, fig.cap = "Density plots showing distributions of responses regarding the three plots"}
...
As in part 1, the two height difference perception questions will be compared, calculating $\bar{x}_{default} - \bar{x}_{narrow}$ and $\bar{x}_{default} - \bar{x}_{wide}$, shown in table 3.3.
...
As before, figure 3.16 below gives a visual representation.
...
```{r plt16, fig.cap= "Bar plot giving a visual representation of the table"}
ggplot()+
...
Both by eye comparisons of values and statistical testing show that the language used has negligible effect on the perceived difference, as does the order in which the plots were shown. See tables in appendix 5 for more details.
...
ratio_3_all <- cbind(default, wide, narrow)
```
...
```{r plt17, fig.cap = "Box plots showing distributions of responses regarding the three plots"}
ggplot(data = stats, aes(x=type, y=resp, fill=type))+
  geom_boxplot(outlier.colour="black", outlier.shape=1,
             outlier.size=2, notch=F)+
  theme_classic()+
...
As seen in figure 3.18, the distributions for the default and narrow plot responses are similar, both seeming to be fairly centered on the mean with a steep decrease in density on either side of the mean to shallow tails within the range $[25, 30]$. The responses for the wide plot appear to be more spread with lower density function values, with a slight negative skew.
...
The last set of questions in part 2 show respondents all three of the bar plots presented in this section and ask them to select which they find most aesthetically pleasing, and which they find easiest and hardest to interpret. table 3.4 gives the number of respondents that selected each plot for each of the three questions.
...
```{r tab4}
kable(tab, caption="Numbers of responses for each option") %>%
  kable_styling(latex_options = "hold_position")
```
...
```{r plt20, fig.cap = "Box plots showing distributions of responses regarding the two plots"}
...
To see if either of these values are significantly far from the true value, tests are once again run. A sign test on the stacked bar plot responses gives a high p-value of 0.5258, showing that for the stacked bar plot responses (after removing the values as priorly specified), the participant estimated values do not differ significantly from the true value. For the grouped bar plot the obtained p-value is 0.009 < 0.05, and thus these responses are statistically significantly different from the true value. 
...
```{r plt21, fig.cap = "Bar plots showing distributions of responses regarding the two plots"}
ggplot(data = stats_2)+
  geom_bar(aes(x=resp, group = type, fill = type), position = 'dodge')+
  theme_classic()+
  scale_fill_manual(breaks = brks, values = vals)+
...
Indeed, the distributions of values for each of the two response sets appear to be almost identical in figure 3.20
...
The respondents were then asked to \textit{'Please select the statement you feel applies to the bar chart above.'}. This question asked respondents to judge whether log grip was used more, less, or an equal amount in the Finals (Regional/City) and Qualifying(Regional/City) rounds. This was to see how well differences between sizes of categories are judged when relating to the same variable, and are in the same bar. The results for this are given table 3.5.
...
grouped_3 <- na.exclude(c(vir_grouped_3, def_grouped_3, gr_grouped_3))
```
...
```{r tab5}
kable(tab, caption="Number of respondents selecting each option for the stacked and grouped bars")%>%
  kable_styling(latex_options = "hold_position")
```

The table shows overwhelmingly that significantly more people accurately judged that the two values were the same for the grouped bars than for the stacked bars. This was the hypothesised result, and has presented to an  even greater extent than previously anticipated. All but 7 of the respondents who responded to this question correctly judged from the grouped bars that the obstacle was used an equal number of times in each of the two rounds, whereas the responses for the grouped bar seemed fairly well split between the three options. It may be interesting in the multivariate analysis section to compare responses depending on whether respondents were shown the stacked or grouped bars first. 

Perhaps a reason for the incorrect judging with the stacked is that the human brain works best when dealing with comparison in position than with length, by the '10 elementary tasks' idea put forward by @clevelandmcgill, since comparing two height next to each other is a comparison in position as opposed to length, whereas the stacked bars are a length comparison.
...
kable(tab, caption="Number of respondents finding each of the two charts easier to read and interpret")%>%
  kable_styling(latex_options = "hold_position")
```
Seen in table 3.6 large majority of participants found the grouped bar chart easier to read and interpret, as predicted.
...
knitr::kable(df, caption = "Colour pairings")%>%
  kable_styling(latex_options = "hold_position")
```
...
a_1_r <- set_a_r$a_cols_1
b_1_r <- set_b_r$b_cols_1
c_1_r <- set_c_r$c_cols_1
d_1_r <- set_d_r$d_cols_1
e_1_r <- set_e_r$e_cols_1
f_1_r <- set_f_r$f_cols_1

a_1_py <- set_a_py$a_cols_1
b_1_py <- set_b_py$b_cols_1
c_1_py <- set_c_py$c_cols_1
d_1_py <- set_d_py$d_cols_1
e_1_py <- set_e_py$e_cols_1
f_1_py <- set_f_py$f_cols_1
...
kable(tab, caption = "Easiest colour scheme to read and interpret")%>%
  kable_styling(latex_options = "hold_position")

tab <- rbind(table(a_1_r), table(b_1_r), table(c_1_r), table(d_1_r), c(table(e_1_r), 0), table(f_1_r))
rownames(tab) <- c("Set A", "Set B", "Set C", "Set D", "Set E", "Set F")
kable(tab, caption = "Easiest to read and interpret colour scheme, for R")%>%
  kable_styling(latex_options = "hold_position")

tab <- rbind(table(a_1_py), table(b_1_py), table(c_1_py), table(d_1_py), c(table(e_1_py), 0), table(f_1_py))
rownames(tab) <- c("Set A", "Set B", "Set C", "Set D", "Set E", "Set F")
```

```{r tab7}
kable(tab, caption = "Easiest to read and interpret colour scheme, for Python")%>%
  kable_styling(latex_options = "hold_position")
```
This table, table 3.7, shows that when it came to the default/viridis pairings, displayed in the first two rows, the respondents tended to have no preference overall. Comparing this to the bottom two rows, in which viridis is put against greyscale, only 1 respondent out of the 23, a proportion of 0.04, found the grey more aesthetically pleasing, as hypothesised. When considering greyscale/default, there was still a majority preferring the non-greyscale palette, but a higher proportion preferred this as compared to the viridis/greyscale, with 4 out of the 22, or a proportion of 0.18, preferring the grey. Overall, 35 preferred viridis, 30 the default, and 5 the greyscale.

As anticipated, the two more-colourful palettes are preferred aesthetically over the grey, and the viridis was preferred over the default.
...
for(i in 1:length(a_2)){
  if(a_2[i] == "Yes, A is easier"){
    a_2[i] <- "A"
  } else if(a_2[i] == "Yes, B is easier"){
  a_2[i] <- "B"
  } else a_2[i] <- "None"
}

for(i in 1:length(b_2)){
  if(b_2[i] == "Yes, B is easier"){
    b_2[i] <- "B"
  } else if(b_2[i] == "Yes, A is easier"){ 
    b_2[i] <- "A"
    }else b_2[i] <- "None"
}

for(i in 1:length(e_2)){
  if(e_2[i] == "Yes, A is easier"){
    e_2[i] <- "A"
  } else if(e_2[i] == "Yes, B is easier"){
    e_2[i] <- "B"
  } else e_2[i] <- "None"
}

for(i in 1:length(f_2)){
  if(f_2[i] == "Yes, B is easier"){
    f_2[i] <- "B"
  } else if(f_2[i] == "Yes, A is easier"){
    f_2[i] <- "A"
  } else f_2[i] <- "None"
}

for(i in 1:length(c_2)){
  if(c_2[i] == "Yes, A is easier"){
    c_2[i] <- "A"
  } else if(c_2[i] == "Yes, B is easier"){ 
    c_2[i] <- "B"
  } else c_2[i] <- "None"
}

for(i in 1:length(d_2)){
  if(d_2[i] == "Yes, A is easier"){
    d_2[i] <- "A"
  } else if(d_2[i] == "Yes, B is easier"){ 
    d_2[i] <- "B"
  } else d_2[i] <- "None"
}
...
for(i in 1:length(a_2_r)){
  if(a_2_r[i] == "Yes, A is easier"){
    a_2_r[i] <- "A"
  } else if(a_2_r[i] == "Yes, B is easier"){
  a_2_r[i] <- "B"
  } else a_2_r[i] <- "None"
}

for(i in 1:length(b_2_r)){
  if(b_2_r[i] == "Yes, B is easier"){
    b_2_r[i] <- "B"
  } else if(b_2_r[i] == "Yes, A is easier"){ 
    b_2_r[i] <- "A"
    }else b_2_r[i] <- "None"
}

for(i in 1:length(e_2_r)){
  if(e_2_r[i] == "Yes, A is easier"){
    e_2_r[i] <- "A"
  } else if(e_2_r[i] == "Yes, B is easier"){
    e_2_r[i] <- "B"
  } else e_2_r[i] <- "None"
}

for(i in 1:length(f_2_r)){
  if(f_2_r[i] == "Yes, B is easier"){
    f_2_r[i] <- "B"
  } else if(f_2_r[i] == "Yes, A is easier"){
    f_2_r[i] <- "A"
  } else f_2_r[i] <- "None"
}

for(i in 1:length(c_2_r)){
  if(c_2_r[i] == "Yes, A is easier"){
    c_2_r[i] <- "A"
  } else if(c_2_r[i] == "Yes, B is easier"){ 
    c_2_r[i] <- "B"
  } else c_2_r[i] <- "None"
}

for(i in 1:length(d_2_r)){
  if(d_2_r[i] == "Yes, A is easier"){
    d_2_r[i] <- "A"
  } else if(d_2_r[i] == "Yes, B is easier"){ 
    d_2_r[i] <- "B"
  } else d_2_r[i] <- "None"
}


for(i in 1:length(a_2_py)){
  if(a_2_py[i] == "Yes, A is easier"){
    a_2_py[i] <- "A"
  } else if(a_2_py[i] == "Yes, B is easier"){
  a_2_py[i] <- "B"
  } else a_2_py[i] <- "None"
}

for(i in 1:length(b_2_py)){
  if(b_2_py[i] == "Yes, B is easier"){
    b_2_py[i] <- "B"
  } else if(b_2_py[i] == "Yes, A is easier"){ 
    b_2_py[i] <- "A"
    }else b_2_py[i] <- "None"
}

for(i in 1:length(e_2_py)){
  if(e_2_py[i] == "Yes, A is easier"){
    e_2_py[i] <- "A"
  } else if(e_2_py[i] == "Yes, B is easier"){
    e_2_py[i] <- "B"
  } else e_2_py[i] <- "None"
}

for(i in 1:length(f_2_py)){
  if(f_2_py[i] == "Yes, B is easier"){
    f_2_py[i] <- "B"
  } else if(f_2_py[i] == "Yes, A is easier"){
    f_2_py[i] <- "A"
  } else f_2_py[i] <- "None"
}

for(i in 1:length(c_2_py)){
  if(c_2_py[i] == "Yes, A is easier"){
    c_2_py[i] <- "A"
  } else if(c_2_py[i] == "Yes, B is easier"){ 
    c_2_py[i] <- "B"
  } else c_2_py[i] <- "None"
}

for(i in 1:length(d_2_py)){
  if(d_2_py[i] == "Yes, A is easier"){
    d_2_py[i] <- "A"
  } else if(d_2_py[i] == "Yes, B is easier"){ 
    d_2_py[i] <- "B"
  } else d_2_py[i] <- "None"
}


tab <- rbind(table(a_2), c(table(b_2)[1], 0,table(b_2)[1]), c(table(c_2), 0), c(table(d_2), 0), c(table(e_2), 0), table(f_2))
rownames(tab) <- c("Set A", "Set B", "Set C", "Set D", "Set E", "Set F")
colnames(tab) <- c("A", "B", "None")
kable(tab, caption = "Aesthetic preference of colour schemes")%>%
  kable_styling(latex_options = "hold_position")

tab <- rbind(table(a_2_r), c(table(b_2_r)[1], 0, table(b_2_r)[2]), c(table(c_2_r), 0), c(table(d_2_r), 0), c(table(e_2_r), 0, 0), table(f_2_r))
rownames(tab) <- c("Set A", "Set B", "Set C", "Set D", "Set E", "Set F")
colnames(tab) <- c("A", "B", "None")
kable(tab, caption = "Aesthetic preference of colour schemes, for R")%>%
  kable_styling(latex_options = "hold_position")

tab <- rbind(table(a_2_py), c(table(b_2_py)[1], 0, 0), c(table(c_2_py), 0, 0), c(table(d_2_py), 0), c(table(e_2_py), 0, 0), c(0, table(f_2_py), 0))
rownames(tab) <- c("Set A", "Set B", "Set C", "Set D", "Set E", "Set F")
colnames(tab) <- c("A", "B", "None")
```

```{r tab8}
kable(tab, caption = "Aesthetic preference of colour schemes, for Python")%>%
  kable_styling(latex_options = "hold_position")
```
Interestingly here, the top two rows of table 3.8 appear to give slightly opposing results; the respondents who were presented with viridis for the main questions and the default as a secondary palette stated that they found either viridis easier to interpret or had no preference, whereas those presented with the default first and viridis second tended to find the default easier. This could perhaps be a result of the respondents becoming used to their primary colour scheme.

Once again looking at the comparisons with the greyscale, there were some respondents that found this easier to read, but the majority chose the alternative, whether this is viridis or the default. 

The results seem fairly similar for the R and Python responses, showing that the default colourings for each language elicit a similar level of ease of interpretation.

The sample of respondents with colour blindness was too small to test this analysis.
...
The plot for which the respondents, on average, found the difference to be smallest was the zeroed, followed by the truncated, and then the separated, with means of 1.371, 2.414 and 3.043 respectively. These differences are found to be statistically significant, as outlined in table 3.9.
...
\section{Conclusion}
From this analysis, it can be concluded that altering axis scales in the way of truncating the axis or converting to a logarithmic scaling may have an effect on interpretation of differences in values, for both bar and line plots. The axis truncation has the effect of increasing the perceived difference in value and the logarithmic does the opposite. In general, from both literature, it is advised against to truncate the axis of a bar plot and this study confirms that it does in fact have an effect on interpretation. A logarithmic scaling may be ill-advised where it will distort the perceived size of the difference in point value, such as for the bar chart used here, however as discussed before from literature could be useful for other purposes, such as data that differs greatly in orders of magnitude. The labeling of this may also need to be considered, since the standard form labeling here confused some respondents. 

Altering the aspect ratios had less of an effect, however there was a marginal effect of the wide plot making the difference in bar height appear smaller, and vice versa for the narrow, with the language not making a huge difference. This means that, while some consideration should be given to aspect ratios when re-scaling plots, it shouldn't have too much of an effect on interpretation.

It was found that grouped bar charts lead to a higher accuracy in interpretation of data values by that a stacked chart, and that the judgment of size difference is also more accurate for the grouped, along with ease of interpretation. Based on this as well as the literature, it appears grouped bars are mostly preferable to stacked. 
