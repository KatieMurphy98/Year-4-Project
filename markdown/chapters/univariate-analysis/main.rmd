...
sum_y
```
We see that removing the outliers as specified by the box plot, the mean of the stacked responses is now just above 11, and actually closer to the true value than the mean of the other set of responses, and the median has decreased to 10. From this one could infer that there is no difference between each type of bar plot in terms of gauging the size of the bars. However, we see that there are 12 outliers in the stacked responses, which leads to the idea that these are not in fact all outliers and may be valid responses that just sit on the upper end of the distribution. The measure for calculating outliers when drawing the box plot may then have misclassified these points. We will decide to keep all values for both types of plot. With this, we can conclude that the respondents appeared to judge the value of the bar better when shown the grouped bars as opposed to the stacked bars, following the earlier defined hypothesis. As expected, removing the outliers also significantly decreases the variances of the two sets of responses.
...
```
Similarly to the previous question, we see that the mean response for the stacked bar plots are higher than that of the grouped, and the mean of the stacked also overestimates the true value. 
